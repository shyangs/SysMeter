...
global version   := "v0.6.1"                ; version number
...
if (showDate = 1)
{
    ...
    */
    Gui, Add, Text, xm ym w100 0x200 vTTIM,
    Gui, Add, Text, xm y+7 w100 0x202 vTUPT,
    Gui, Add, Text, xm y+7 w30 0x200, % "CPU"
}
else
{
    Gui, Add, Text, xm ym w30 0x200, % "CPU" ; `0x200` center text control (height).
}
Gui, Add, Text, x+5 yp w65 0x202 vTCPU, ; `0x2` moves text to right. ; `0x200` + `0x2` = `0x202`.
Gui, Add, Progress, xm y+2 w100 h6 c%cpcpu% Background%cpbg% vPCPU,
Gui, Add, Text, xm y+7 w30 0x200, % "RAM"
Gui, Add, Text, x+5 yp w65 0x202 vTRAM,
Gui, Add, Progress, xm y+2 w100 h6 c%cpmem% Background%cpbg% vPRAM,
Gui, Add, Text, xm y+7 w30 0x200, % "SWAP"
Gui, Add, Text, x+5 yp w65 0x202 vTSWP,
Gui, Add, Progress, xm y+2 w100 h6 c%cpmem% Background%cpbg% vPSWP,
/*
...
Update:
    if (showDate = 1)
    {
        GuiControl,, TTIM, % A_DD ". " A_MM " " A_YYYY "  " A_Hour ":" A_Min ":" A_Sec
        GuiControl,, TUPT, % "UPTIME: " GetDurationFormat(DllCall("Kernel32.dll\GetTickCount64", "UInt64") / 1000) ; https://learn.microsoft.com/zh-tw/windows/win32/api/sysinfoapi/nf-sysinfoapi-gettickcount64 ;.
    }
    ...
    GMSExM05 := Round(GMSExM04 / GMSExM02 * 100, 0)        ; Used Physical Memory in %
    ...
    GMSExS04 := Round(GMSExS03 / GMSExS01 * 100, 0)        ; Used PageFile in %
